...
private[spark] class LiveListenerBus(val sparkContext: SparkContext) extends SparkListenerBus {
  ...
  // 限制事件队列的容量，所以如果永久添加的速度比其耗尽更快，我们会得到一个明确的错误（而不是OOM异常）。
  private lazy val EVENT_QUEUE_CAPACITY = validateAndGetQueueSize()

  // SparkListenerEvent事件的注射队列，队列大小可以通过Spark属性spark.scheduler.listenerbus.eventqueue.size
  // 进行配置，默认为10000
   ...
   *
   * 使用AtomicLong类型对删除的事件进行计数，每当日志打印了
   * droppedEventsCounter，会将droppedEventsCounter重置为0
   */
  ...
  // 用于当有新的事件到来时释放信号量，当对事件进行处理时释放信号量。
  private val eventLock = new Semaphore(0)

  /**
   * 用于异步处理eventQueue中的事件
   */
      ...
      LiveListenerBus.withinListenerThread.withValue(true) {
        while (true) {
          ...
          try {
            ...
            self.synchronized {
              // 通过同步控制，将processingEvent设置为false
  ...
  def post(event: SparkListenerEvent): Unit = {
    ...
    // 如果eventQueue已满，造成添加失败，则移除事件，并对删除事件计数器droppedEventsCounter进行自增。
    ...
    // 如果有事件被删除，并且当前系统时间距离上一次打印droppedEventsCounter超过了60秒，
    // 则将droppedEventsCounter打印到日志。
