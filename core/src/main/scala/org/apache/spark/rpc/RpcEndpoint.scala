 ...
 *
 * RpcEndpoint是对RPC通信实体的统一抽象，所有运行于RPC框架之上的实体都应继承于RpcEndpoint。
 * 和Akka中的Actor类似
 */
private[spark] trait RpcEndpoint {
   ...
   *
   * 获取RpcEndpoint关联的RpcEndpointRef。
   */
 ...
 *
 * 很多Spark中的Endpoint继承了这个类，Spark RPC框架对这种Endpoint不做并发处理，
 * 也就是同一时间只允许一个线程在做调用。
 */
