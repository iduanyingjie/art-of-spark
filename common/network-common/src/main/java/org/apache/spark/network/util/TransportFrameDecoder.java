 ...
 * 一个定制的帧解码器，允许拦截原始数据。
 *
 * 这就像netty的帧解码器一样（带有符合这个库需求的编码参数），除了它允许安装拦截器以便在构建之前直接读取数据。
 *
 * 与netty的帧解码器不同，每一帧在解码时都会被分派给子处理程序，而不是像当前缓冲区允许的那样构建尽可能多的帧并将其全部分配。
 * 这允许子处理程序根据需要安装拦截器。
 *
 * 如果安装了拦截器，则停止组帧，并将数据直接提供给拦截器。
 * 当拦截器指示它不需要读取更多数据时，帧恢复。拦截器不应该持有对提供给它们的handle（）方法的数据缓冲区的引用。
 *
 * art_of_spark: 对管道中读取的ByteBuf按照数据帧进行解析。
 */
