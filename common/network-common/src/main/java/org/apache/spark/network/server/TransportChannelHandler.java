 ...
 *
 * 单个传输级通道处理程序，用于将请求委托给传输请求处理程序和对传输响应处理程序的响应。
 *
 * 在传输层中创建的所有通道都是双向的。
 * 当客户端使用RequestMessage（由服务器的RequestHandler处理）启动一个netty通道时，
 * 服务器将产生一个ResponseHandler（由客户端的ResponseHandler处理）。
 * 但是，服务器也在同一个通道上获得句柄，所以它可能会开始发送请求消息给客户端。
 * 这意味着客户端还需要RequestHandler，并且服务器需要ResponseHandler，以便客户端响应服务器的请求。
 *
 * 这个类还处理来自io.netty.handler.timeout.IdleStateHandler的超时。
 * 如果存在未完成的提取或rpc请求，但通道上没有流量，则至少有'requestTimeoutMs`，我们认为连接超时。
 * 请注意，这是双工流量;为简单起见，如果客户端连续发送但没有收到响应，我们不会超时。
 *
